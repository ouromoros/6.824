...
func (rf *Raft) updateAndSetTimeout() {
	...
	rf.mu.Lock()
	if rf.status != leader && rf.timeOutSeqNum == seq {
		rf.turnCandidate()
	}
	rf.mu.Unlock()
}
...
func (rf *Raft) turnLeader() {
	rf.DPrintf("Server %d has become leader on Term %d", rf.me, rf.currentTerm) rf.currentStatus = leader
